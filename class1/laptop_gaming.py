...
class Laptop_Gaming(Laptop):
    def __init__(self, marca, procesador, memoria, tarj_graf, costo=500, impuesto=10):
        ...
        self.tarj_graft = tarj_graf

    def __str__(self):
        return f"Marca: {self.marca} \n Procesador= {self.procesador} \n Memoria= {self.memoria} \n Tarjeta Grafica: {self.tarj_graft} \n Costo: {self.costo} \n Impuesto: {self.impuesto}"
    ...
    def realizar_diagnostico_juegos(self):
        juegos = ["Fortnite", "Minecraft", "Call of Duty", "FIFA"]
        resultados = {}
        for juego in juegos:
            fps_base = 30
            if "RTX" in self.tarj_graft:
                fps = fps_base * 3
            elif "GTX" in self.tarj_graft:
                fps = fps_base * 2
            else:
                fps = fps_base
